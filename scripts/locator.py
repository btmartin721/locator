...
import allel, os, sys, zarr, time, subprocess, copy, pickle
...
class MLPRegressor(KerasRegressor):
    ...
    def _keras_build_fn(self, compile_kwargs):
        ...
        model = Sequential()
        model.add(BatchNormalization(input_shape=(self.n_features_in_,)))
        for _ in range(self.nlayers):
            model.add(
                Dense(
                    self.width,
                    activation="elu",
                    kernel_regularizer=L1L2(l1=self.l1_weight, l2=self.l2_weight),
                )
            )
...
def train_network(model, traingen, testgen, trainlocs, testlocs):
    ...
    with open(f"{args.out}_best_score.txt", "w") as fout:
        fout.write(str(grid_result.best_score_))

    with open(f"{args.out}_best_params.txt", "wb") as fout:
        pickle.dump(str(grid_result.best_params_), fout)
